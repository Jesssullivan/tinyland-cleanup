...
require_contains "$workflow" "if: github.ref == 'refs/heads/main'" "protected-main-only proof gate"
require_contains "$workflow" 'nix develop --command bash scripts/bazel-cache-backed.sh' "lock-pinned public proof toolchain"
reject_contains "$workflow" 'nix shell nixpkgs#' "floating Nix tool input"
